...
export default function AboutPage() {
  ...
  return (
    <>
      ...
      <section aria-labelledby="about-company-heading" className="bg-white pb-20 lg:pb-24">
        <div className="mx-auto max-w-[1400px] px-6 lg:px-12">
          <div className="grid grid-cols-1 gap-14 lg:grid-cols-12 lg:gap-10">
            <div className="lg:col-span-6">
              ...
              <p className="mt-6 text-[15.5px] leading-relaxed text-ink-soft">{t.companyBody}</p>

              <ul className="mt-10 grid grid-cols-1 gap-px border border-line bg-line sm:grid-cols-2">
                {dict.services.list
                  .filter((s) => s.priority === "primary")
                  .map((service) => (
                    <li key={service.slug} className="bg-paper p-6">
                      <span className="font-display text-[11px] font-semibold tracking-[0.2em] text-mute tabular-nums">{service.index}</span>
                      <span className="mt-3 block text-[14.5px] leading-snug font-medium tracking-tight text-navy-900">{service.title}</span>
                      <span className="mt-2 block text-[12.5px] leading-snug text-mute">{service.tag}</span>
                    </li>
                  ))}
              </ul>
            </div>
            ...
            <div className="lg:col-span-5 lg:col-start-8">
              ...
              <dl className="mt-6 border-t border-line">
                {facts.map((fact) => (
                  <div key={fact.term} className="border-b border-line py-5">
                    <dt className="text-[10.5px] font-medium tracking-[0.14em] text-mute uppercase">{fact.term}</dt>
                    <dd className="mt-1.5 text-[14px] leading-relaxed text-ink">{fact.value}</dd>
                  </div>
                ))}
                <div className="border-b border-line py-5">
                  <dt className="text-[10.5px] font-medium tracking-[0.14em] text-mute uppercase">{legal.activities}</dt>
      ...
      <section aria-labelledby="about-leadership-heading" className="bg-paper py-20 lg:py-28">
        <div className="mx-auto max-w-[1400px] px-6 lg:px-12">
          <div className="grid grid-cols-1 gap-14 lg:grid-cols-12 lg:gap-10">
            <div className="lg:col-span-4">
              ...
              <p className="mt-6 text-[15px] leading-relaxed text-ink-soft">{leadership.lede}</p>
            </div>

            <div className="lg:col-span-7 lg:col-start-6">
              <ul className="border-t border-line">
                {leadership.members.map((member, i) => (
                  <Reveal as="li" key={member.name} delay={i * 80} className="border-b border-line">
                    <div className="flex flex-col gap-4 py-8 sm:flex-row sm:items-start sm:gap-7">
                      <span className="flex h-14 w-14 shrink-0 items-center justify-center border border-line bg-white font-display text-[14px] font-semibold tracking-wide text-navy-900" aria-hidden="true">
                        {member.initials}
                      </span>
                      <div className="flex-1">
                        ...
                        {member.note && <p className="mt-2 max-w-xl text-[14.5px] leading-relaxed text-ink-soft">{member.note}</p>}
                      </div>
                    </div>
                  </Reveal>
                ))}
              </ul>
              <p className="mt-5 text-[13px] leading-relaxed text-mute">{leadership.footnote}</p>
            </div>
          </div>

          <div className="mt-16 border border-line bg-white px-8 py-12 lg:px-14">
            <div className="grid grid-cols-1 items-center gap-10 lg:grid-cols-12">
              <div className="lg:col-span-7">
                ...
                <p className="mt-5 max-w-2xl text-[14.5px] leading-relaxed text-ink-soft">{t.ctaBody}</p>
              </div>
              <div className="flex flex-wrap items-center gap-4 lg:col-span-5 lg:justify-end">
                ...
                <Link
                  to={PATHS.services}
                  className="inline-flex items-center gap-2 border border-line-strong px-6 py-3.5 text-[13px] font-medium tracking-wide text-navy-900 transition-colors hover:border-navy-900 hover:bg-paper"
                >
                  {c.services}
                </Link>
              </div>
